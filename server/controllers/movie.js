...
class MovieController {
  async recommender(req, res) {
    try {
      const movieId = req.params.movie_id;
      const movies = await Movie.find({});

      const dataSets = [movieId, movies];

      const pythonProcess = spawn("python", ["services/recommender.py"]);

      dataSets.forEach((data) => {
        const dataSerialzed = JSON.stringify(data);
        pythonProcess.stdin.write(dataSerialzed + "\n");
      });
  ...
  async getSimilar(req, res) {
    try {
      ...
      console.log("similarMovie", movie);
